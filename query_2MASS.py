...
def geturl(ra, dec, size=150, filters="A"):
    ...
    # url for querying 
    url = ("https://irsa.ipac.caltech.edu/cgi-bin/2MASS/IM/nph-im_sia?POS={pos}"
       "&SIZE={size}&band={band}&ds={survey}".format(**locals()))
    url += "&INTERSECT=CENTER" # returned files must overlap center of ROI 
    ...
    f_urls = []
    for f in f_messy:
        if len(f) != 0: # files_messy is sometimes pop. with empty strings 
            f_urls.append(f+".fits") # .fits needs to be appended to all files
